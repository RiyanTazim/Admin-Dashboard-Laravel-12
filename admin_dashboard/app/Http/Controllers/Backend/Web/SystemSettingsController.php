...
use App\Models\SystemSettings as Settings;
use Exception;
...
class SystemSettingsController extends Controller
{
    public function systemIndex()
    {
        ...
        return view('Backend.Layout.admin.systemSetting', compact('setting'));
    }
        ...
        public function systemSettings(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('logo')) {
                if (! empty($setting->logo) && file_exists(public_path($setting->logo)) && $setting->logo != 'images/systems/logo/logo.png') {
                    unlink(public_path($setting->logo));
                }

                $logoPath = public_path('images/systems/logo');
                ...
                $setting->logo = 'images/systems/logo/' . $filename;
            }

            // Handle favicon
            if ($request->hasFile('favicon')) {
                if (! empty($setting->favicon) && file_exists(public_path($setting->favicon)) && $setting->favicon != 'images/systems/favicon/favico.png') {
                    unlink(public_path($setting->favicon));
                }

                $faviconPath = public_path('images/systems/favicon');
                ...
                $setting->favicon = 'images/systems/favicon/' . $filename;
            }
