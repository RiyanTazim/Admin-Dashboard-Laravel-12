@php
    ...
    $logoPath =
        isset($setting->logo) && file_exists(public_path($setting->logo)) ? asset($setting->logo) : $defaultLogo;
@endphp
...
<div class="sticky">
    ...
    <div class="app-sidebar">
        <div class="side-header">
            <a class="header-brand1 ms-0" href="{{ route('dashboard') }}">
                {{-- <img src="{{ $logoPath }}" width="40" alt="logo"> --}}
                <h5 class="brand-text">{{ $setting->admin_title ?? ' My Admin ' }}</h5>
            </a>
        </div>
            ...
            <ul class="side-menu">
                ...
                <li class="slide">
                    <a class="side-menu__item" data-bs-toggle="slide" href="#">
                        ...
                        <span
                            class="side-menu__label {{ request()->routeIs(['admin.*', 'system.*']) ? 'active' : '' }}">Settings</span><i
                            class="angle fa fa-angle-right"></i>
                    </a>
                    <ul class="slide-menu">
                        {{-- <li class="side-menu-label1"><a href="javascript:void(0)">System Settings</a></li> --}}
                        <li><a href="{{ route('admin.index') }}"
                                class="slide-item {{ request()->routeIs('admin.index') ? 'active' : '' }}">Admin Settings</a></li>
                        <li><a href="{{ route('system.index') }}"
                                class="slide-item {{ request()->routeIs('system.index') ? 'active' : '' }}">System Settings</a></li>
                        <li><a href="{{ route('system.index') }}"
                                class="slide-item {{ request()->routeIs('system.index') ? 'active' : '' }}">Mail Settings</a></li>
                        <li><a href="{{ route('system.index') }}"
                                class="slide-item {{ request()->routeIs('system.index') ? 'active' : '' }}">Role Settings</a></li>
                        <li><a href="{{ route('system.index') }}"
                                class="slide-item {{ request()->routeIs('system.index') ? 'active' : '' }}">User Settings</a></li>
                    </ul>
                </li>
            </ul>
        </div>
